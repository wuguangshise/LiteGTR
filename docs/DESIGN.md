...
|---|---|---|
| tiny GT gets no candidate | NWD (ISPRS'22): assignment is where NWD helps most -- IoU matching leaves 0.72 positives per tiny GT vs 1.05 with NWD. **RFLA** (ECCV'22): Gaussian receptive field vs Gaussian GT, KL-based distance, hierarchical top-k. **STAL** (Ultralytics YOLO26): widen tiny GTs for candidate selection only, regress the original box | **RFLA is the main model's assigner** (`assigner.mode: rfla`, `assigners/rfla_assigner.py`, ported from the official code with GFL-style soft targets); **TAL + STAL** (`stal_size: 8`) is ablation #6. Whichever wins on AP_vt / AP_t / AP_small becomes the main model. RF sizes: RFLA's own P2 setting (17.5 / 45.5 / 133.5 / 213.5 px per stride 4-32) -- TinyNeXt's theoretical RF halved would route every object under ~75 px to P2 and starve the token levels. RFLA gives ~4 positives per GT (3 + 1) against TAL's up to 13 |
