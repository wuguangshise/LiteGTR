...
C.push(P("并与 GIoU 损失共同优化。正样本由 TaskAligned 分配器选取，对齐度量为 *t* = *s*^{α}·*u*^{β}，其中 *s* 为分类分数、*u* 为 IoU，取 α = 1、β = 6、每个真值取 top-13 候选。候选点必须位于真值框内部；在 640 输入下，边长小于 P2 步长（4 像素）的目标可能落在网格中心之间而没有任何候选点，按公开尺度统计估算约占全部真值的 9.5%，这些目标从未成为正样本，反而被当作背景训练。我们采用 YOLO26 的小目标感知分配（STAL）：仅在候选点选取时，把短于 8 像素的边以中心为基准扩展到 8 像素，回归目标仍为原始框；边长不小于 8 像素的真值，分配结果不变。"));
...
C.push(P("**VisDrone2019-DET**：6471 张训练、548 张验证图像，10 类。ignored 区域以填充值屏蔽，others 类剔除。所有消融在 val 上报告。**DroneVehicle**：使用 RGB 模态、水平框，按亮度把图像标注为 day / night / dark，用于跨光照分析。评测使用 COCO API（maxDets = 100），另按 3.1 节分档报告 AP_{vt}、AP_{t}。后处理与 RemDet、Ultralytics 的评测设置一致：分数阈值 0.001，每个位置可输出多个类别，按类别做 IoU 阈值 0.7 的 NMS，每图至多保留 300 个框。同量级模型的外部结果评测协议不一，单独标注。"));
C.push(H2("5.2  实现细节"));
