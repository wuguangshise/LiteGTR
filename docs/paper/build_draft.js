...
C.push(P("并与 GIoU 损失共同优化。正样本由 TaskAligned 分配器选取，对齐度量为 *t* = *s*^{α}·*u*^{β}，其中 *s* 为分类分数、*u* 为 IoU，取 α = 1、β = 6、每个真值取 top-13 候选。"));
C.push(H2("3.3  无人机图像的尺度统计"));
C.push(P("表 1 给出本文预处理（屏蔽 ignored 区域、去除 others 类）后 VisDrone 的尺度统计，分别在原图尺度和 640 输入尺度下计算。网络实际看到的是后者：在 640 输入下，[xx]% 的目标边长小于一个 P3 单元（8 像素）。这是本文在 P3 上分配最多 token、并用 P2 保留局部细节的依据。"));
C.push(...TABLE("VisDrone 尺度统计（本文预处理后）。按目标边长 √a（像素）分档，分别在原图尺度与 640 输入尺度下统计。", 
  ["划分", "尺度", "图像", "实例", "目标/图（均值 / 中位 / 最大）", "<8", "8–16", "16–32", "32–64", "≥64"],
  [["train", "原图", "6471", "—", "— / — / —", "—", "—", "—", "—", "—"],
   ["train", "640 输入", "6471", "—", "— / — / —", "—", "—", "—", "—", "—"],
   { cells: ["val", "原图", "548", "—", "— / — / —", "—", "—", "—", "—", "—"], sep: true },
   ["val", "640 输入", "548", "—", "— / — / —", "—", "—", "—", "—", "—"]],
  [8, 9, 7, 8, 22, 7, 7, 7, 7, 7],
  "注：由 tools/analyze_dataset.py 统计。参考值：公开统计中 VisDrone val 面积 < 16² 的目标约占 30.8%，< 32² 的约占 68.5%，大目标约占 2.8%。"));
C.push(...FIG("VisDrone 目标边长分布（原图尺度与 640 输入尺度）。竖线标出 P2–P5 的步长 4/8/16/32 像素。", [
  "内容：两条直方图（原图 / 640 输入），横轴目标边长（对数坐标），纵轴实例占比；竖虚线标注 stride 4、8、16、32。",
  "数据来源：tools/analyze_dataset.py 输出。"], 2000));

// ---------------- 4 Method
C.push(H1("4  方法"));
C.push(H2("4.1  总体结构"));
C.push(P("LiteGTR 的结构如图 3 所示。TinyNeXt 骨干提取 *C*_{2}–*C*_{5}，经金字塔投影与 FPN 得到 P2–P5，每层再经过局部卷积路径。P3–P5 进入 token 路径：固定预算局部路由选出 56 个 token，token 混合器在 token 之间建模全局关系，几何感知写回把结果以残差形式写回 P3–P5。P2 与写回后的 P3′–P5′ 送入 GFL 检测头。路由监督和 EMA 教师只在训练时使用，不参与推理。"));
C.push(...FIG("LiteGTR 总体结构。(a) 整体流程；(b) TinyNeXt Block；(c) 固定预算局部路由；(d) 几何感知写回；(e) 仅训练使用的路由监督与光照一致 EMA 教师（虚线）。", [
  ...
  "(c) 4×4 / 4×4 / 2×2 网格，每区域 top-k 白点。(d) 内容项 ⊕ 高斯先验 → softmax → Σ → γ·Proj → ⊕；配各向异性高斯椭圆示意。",
...
C.push(...FIG("几何感知写回示意。不同 token 预测的高斯范围不同：落在小目标上的 token 范围窄，只作用于邻域；描述背景与场景的 token 范围宽。", [
  "内容：一张 P3 特征图网格上叠加若干高斯椭圆（窄：车辆、行人；宽：道路、建筑），每个椭圆中心为 token 位置 c_{i}，标注 σ_{i}。",
  "右侧：同一位置 p 的写回权重分解为 内容项 C(p,i) + 几何项 G(p,i)。"], 2400));
...
C.push(P("P3 在 640 输入下有 6400 个位置。对其做全局自注意力的代价为 O((HW)^{2}C)；LiteGTR 中 token 混合为 O(*N*^{2}*C*)，写回为 O(*N*·HW·*C*)，*N* = 56。路由的 *k*、网格划分与所有张量形状均为常量，ONNX 以静态形状导出，测试中若出现动态维度即判为失败。表 2 给出各模块的参数与计算量。"));
C.push(...TABLE("LiteGTR 各模块参数量（640×640 输入）。参数量不含仅训练使用的 EMA 教师（56K）。",
  ["模块", "参数量", "说明"],
  [["TinyNeXt 骨干", "2.03M", "[32,64,128,192] × [2,4,8,2]"],
   ["金字塔投影 + FPN", "47K", "64 通道"],
   ["局部卷积路径", "34K", "P2 ×1，P3–P5 ×2 DWSep"],
   ["Token 打分器", "56K", "P3/P4/P5 各一个"],
   ["Token 混合器", "34K", "1 层，单头，FFN ×2"],
   ["几何写回", "50K", "P3–P5，各层独立"],
   ["GFL 检测头", "74K", "P2–P5，reg_max = 16"],
   { cells: ["合计（LiteGTR）", "2.32M", "4.93G MACs（≈9.9 GFLOPs），另含注意力矩阵乘约 0.06G"], bold: true, sep: true },
   ["合计（LiteGTR-S）", "1.22M", "2.49G MACs（≈5.0 GFLOPs）"]],
  [26, 14, 60], "注：由 tools/profile_model.py（thop）测得；thop 不统计注意力中的矩阵乘，已单独给出。"));
...
C.push(...TABLE("VisDrone2019-DET val 上的比较。上半部分为引用的已发表结果（评测协议与训练轮数各不相同，仅供参考）；下半部分为本文在统一协议下训练与评测。",
  ["方法", "来源", "轮数", "Params(M)", "GFLOPs", "mAP_{50:95}", "mAP_{50}", "AP_{S}", "延迟(ms)"],
  [{ group: "引用结果†" },
   ["LEAF-YOLO-N", "ISWA'25", "—", "1.2", "5.6", "21.9", "39.7", "14.0", "16.2^{a}"],
   ["EDNet-Tiny", "UIC'24", "—", "1.8", "—", "19.5", "34.1", "—", "2.0^{b}"],
   ["YOLO26n-P2", "DHF-YOLO", "300", "2.40", "6.5", "19.2", "33.1", "—", "—"],
   ["DHF-YOLO", "2026", "300", "2.63", "9.9", "20.4", "34.9", "—", "—"],
   ["RemDet-Tiny", "AAAI'25", "300", "3.2", "4.6^{c}", "21.8", "37.1", "12.7", "—"],
   ["LEAF-YOLO", "ISWA'25", "—", "4.28", "20.9", "28.2", "48.3", "20.0", "21.7^{a}"],
   ["EdgeYOLO-Tiny", "—", "—", "5.5", "27.2", "21.8", "38.5", "12.4^{d}", "—"],
   ["PDWT-YOLO", "—", "—", "6.44", "24.5", "24.3", "42.6", "15.9^{d}", "—"],
  ...
  [20, 11, 7, 10, 9, 11, 9, 8, 10],
  "† 数据取自作者公开的代码仓库 README 或训练日志。a：RTX 3090，PyTorch FP16，batch 1；b：A100，TensorRT；c：mmdet 统计口径，可能为 MACs；d：由 LEAF-YOLO 作者测得。YOLOv8n/YOLO11n 的 Params/GFLOPs 按 10 类模型实测填写。空白处待实验填写。"));
C.push(...TABLE("细分尺度精度（VisDrone val）。所有模型用同一评测脚本计算；外部模型使用其公开权重重新评测。",
  ["方法", "Params(M)", "AP_{vt}", "AP_{t}", "AP_{s}", "AP_{m}", "AP_{S}（COCO）"],
  [["LEAF-YOLO-N（公开权重）", "1.2", "", "", "", "", ""],
   ["RemDet-Tiny（公开权重）", "3.2", "", "", "", "", ""],
   { cells: ["CSP-N", "2.25", "", "", "", "", ""], sep: true },
   ["LiteGTR w/o token", "—", "", "", "", "", ""],
   { cells: ["LiteGTR", "2.32", "", "", "", "", ""], bold: true }],
  [30, 12, 10, 10, 10, 10, 14],
  "注：AP_{vt} / AP_{t} / AP_{s} / AP_{m} 按边长 [2,8) / [8,16) / [16,32) / [32,64) 像素分档（AI-TOD 定义），按原图坐标统计。"));
C.push(TODO("结果分析：与参数量匹配基线的差距；与引用结果的比较需说明协议差异。"));

C.push(H2("5.4  消融实验"));
C.push(...TABLE("组件消融（VisDrone val）。每行相对完整模型只改变一项。",
  ["配置", "Params(M)", "GFLOPs", "mAP_{50:95}", "mAP_{50}", "AP_{S}", "AP_{M}", "AP_{L}"],
  [{ cells: ["LiteGTR（完整）", "2.32", "≈9.9", "", "", "", "", ""], bold: true },
   ["− Token 路径（仅局部卷积）", "", "", "", "", "", "", ""],
   ["− 几何先验（仅内容注意力）", "2.32", "", "", "", "", "", ""],
   ["− 路由监督", "2.32", "≈9.9", "", "", "", "", ""],
   ["− 光照一致 EMA", "2.32", "≈9.9", "", "", "", "", ""]],
  [34, 10, 9, 11, 9, 9, 9, 9],
  "注：AP_{M} 与 AP_{L} 在此给出，用于量化面向小目标设计对中、大目标的影响；VisDrone val 中大目标仅约占 2.8%。"));
C.push(...TABLE("路由行为分析。命中率为被选中 token 落在真值框内的比例；随机水平为目标面积占比。",
  ["路由方式", "分数图熵（P3）↓", "Token 命中率 ↑", "随机水平", "EMA 一致性", "AP_{S}"],
  [["随机路由（无门控、无监督）", "—", "", "", "—", ""],
...
C.push(...TABLE("写回方式对比。",
  ["写回方式", "mAP_{50:95}", "AP_{S}", "AP_{vt}", "AP_{L}"],
  [["均匀广播", "", "", "", ""], ["内容注意力", "", "", "", ""], { cells: ["内容 + 高斯几何先验（本文）", "", "", "", ""], bold: true }],
  [40, 15, 15, 15, 15]));
C.push(...TABLE("Token 预算。",
  ["预算（P3 / P4 / P5）", "N", "GFLOPs", "延迟(ms)", "mAP_{50:95}", "AP_{S}"],
  [{ cells: ["32 / 16 / 8（本文）", "56", "≈9.9", "", "", ""], bold: true }, ["128 / 96 / 32", "256", "", "", "", ""]],
  [30, 10, 14, 14, 16, 16]));
C.push(TODO("消融分析。重点：− token 路径时 AP_{S} 与 AP_{L} 的变化方向；无监督路由的命中率是否接近随机。"));
C.push(...FIG("Token 分数图与被选中位置的可视化（VisDrone val）。上：无路由监督；下：本文。颜色为 σ(s) 的绝对值（0–1），白点为被选中的 token。", [
  "由 tools/visualize_tokens.py 生成：输入+GT | P3 | P4 | P5 分数图 | 写回变化量。",
  "选 2–3 张目标密集、小目标多的图像。"], 2600));
C.push(...FIG("训练过程中 P3 分数图归一化熵的变化。无监督时熵单调趋近 1。", [
  "数据来源：两次训练的 token_stats.csv（epoch, score_entropy）。"], 1800));

C.push(H2("5.5  跨光照分析"));
C.push(...TABLE("DroneVehicle（RGB）上按光照条件拆分的 mAP_{50:95}。",
  ["配置", "day", "night", "dark", "Δ(night − day)"],
  [{ cells: ["LiteGTR", "", "", "", ""], bold: true }, ["− Token 路径", "", "", "", ""], ["− 光照一致 EMA", "", "", "", ""],
   { cells: ["仅 IR（参考）", "", "", "", ""], sep: true }],
  [34, 16, 16, 16, 18], "注：图像数 day / night / dark = [待补]。"));

C.push(H2("5.6  部署效率"));
C.push(...TABLE("推理效率（640×640，batch 1）。",
  ["模型", "Params(M)", "GFLOPs", "PyTorch FP32", "ONNX Runtime", "TensorRT FP16", "Jetson [型号]"],
  [["CSP-N", "2.25", "≈7.1", "", "", "", ""], ["LiteGTR-S", "1.22", "≈5.0", "", "", "", ""],
   { cells: ["LiteGTR", "2.32", "≈9.9", "", "", "", ""], bold: true }],
  [18, 11, 10, 15, 15, 15, 16], "注：由 tools/benchmark_latency.py 测得，单位 ms；是否包含 NMS 需注明。"));
C.push(...FIG("定性结果对比（VisDrone val，密集小目标场景）。从左到右：GT、CSP-N、LiteGTR w/o token、LiteGTR。", [
  "每列同一张图；红框标注漏检区域的放大图。"], 2200));
